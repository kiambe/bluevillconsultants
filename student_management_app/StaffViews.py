...
from student_management_app.forms import AddFeedbackForm
...
def staff_feedback_save(request):
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            image_update = form.cleaned_data['image_update']
            video_update = form.cleaned_data['video_update']
            status = form.cleaned_data['status']
            staff_obj = Staffs.objects.get(admin=request.user.id)

            if len(request.FILES) != 0:
                image_update = request.FILES['image_update']
                fs = FileSystemStorage()
                filename = fs.save(image_update.name, image_update)
                image_update_url = fs.url(filename)
            else:
                image_update_url = None
            
            try:
                add_feedback = FeedBackStaffs(staff_id=staff_obj, feedback=feedback, feedback_reply="", status=status, project_id=project_id, video_update=video_update, category=category, image_update=image_update_url)
                add_feedback.save()     
                messages.success(request, "Report Added Successfully!")
                return redirect('staff_feedback_report')
            except:
...
def staff_feedback_save2(request):
    if request.method != "POST":
        messages.error(request, "Invalid Method.")
        return redirect('staff_feedback_report')
    else:
        feedback = request.POST.get('feedback_message')
        video_update = request.POST.get('video_update')
        image_update = request.POST.get('image_update')
        ...
        try:
            add_feedback = FeedBackStaffs(staff_id=staff_obj, feedback=feedback, feedback_reply="", video_update=video_update, category=category, image_update=image_update)
            add_feedback.save()
